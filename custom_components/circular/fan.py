...
@dataclass
class CircularFanRequiredKeysMixin:
    """Required keys for fan entity."""

    set_fn: Callable[[CircularApiClient, int], Awaitable]
    value_fn: Callable[[CircularApiData], bool]
    speed_range: tuple[int, int]


@dataclass
class CircularFanEntityDescription(FanEntityDescription, CircularFanRequiredKeysMixin):
    """Describes a fan entity."""


CIRCULAR_FANS: tuple[CircularFanEntityDescription, ...] = (
    CircularFanEntityDescription(
        key="fan",
...
async def async_setup_entry(
    ...
    async_add_entities(
        CircularFan(coordinator=coordinator, description=description)
        for description in CIRCULAR_FANS
    )


class CircularFan(CircularEntity, FanEntity):
    """This is Fan entity for the stove."""

    entity_description: CircularFanEntityDescription
